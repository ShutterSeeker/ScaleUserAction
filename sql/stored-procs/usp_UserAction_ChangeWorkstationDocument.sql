USE [ILS]
GO
/****** Object:  StoredProcedure [dbo].[usp_UserAction_ChangeWorkstationDocument]    Script Date: 4/8/2026 12:55:21 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER   PROC [dbo].[usp_UserAction_ChangeWorkstationDocument] (
...
/*
 ...
 --------------------------------------------------------------------
 Changes non-label related document routing records for a particular workstation
*/
...
DECLARE @documentPrinter NVARCHAR(50)
...
SELECT @templatePrinter = DOCUMENT_PRINTER FROM JP19_WORKSTATION_DOC_VIEW WHERE WORKSTATION = N'*Unassigned'
SELECT @documentPrinter = DOCUMENT_PRINTER FROM JP19_WORKSTATION_DOC_VIEW WHERE WORKSTATION = @internalID

IF @resolvedDeviceName IS NULL OR @resolvedIsLabel = 1
BEGIN
    INSERT INTO @Results (MessageCode, Message)
    VALUES (N'ERR_CHANGEWSDOCUMENT01', N'The selected printer is not a valid document printing device.')
...
BEGIN TRY
    ...
    BEGIN
        UPDATE DOCUMENT_ROUTING SET
            ...
            ,PROCESS_STAMP = N'usp_UserAction_ChangeWorkstationDocument'
    ...
    END ELSE
    BEGIN
        -- Clear out any existing doc routing for this workstation / document printer
        DELETE DOCUMENT_ROUTING
        WHERE MACHINE_NAME = @internalID
            AND DEVICE_NAME = @documentPrinter
    ...
    INSERT INTO @Results (MessageCode, Message)
    VALUES (N'MSG_CHANGEWSDOCUMENT01', N'Document printer successfully changed!')

    DECLARE @msg NVARCHAR(200) = @internalID + N' document printer changed from ' + @documentPrinter + N' to ' + @resolvedDeviceName
    EXEC HIST_SaveProcHist
        ...
        NULL,
        @msg,
        N'usp_UserAction_ChangeWorkstationDocument',
        @userName,
        NULL,
        NULL
END TRY
BEGIN CATCH
    ...
    EXEC ADT_LogAudit
        'usp_UserAction_ChangeWorkstationDocument',
        -1,
        @Error,
        'Action: ', N'ChangeWorkstationDocument',
        'Workstation: ', @internalID,
        'Document Printer: ', @Device,
        'Error: ', @Error,
        'User: ', @userName,
        NULL, NULL,
        NULL, NULL,
